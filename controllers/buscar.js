...
const buscarProductos = async (termino = '', res = response) => {
    ...
    if (esMongoId) {
        ...
        return res.json({ results: (producto) ? [producto] : [] }).populate('categoria', 'nombre');
    }
...
const buscar = (req = request, res = response) => {
    ...
    if (!coleccionesPermitidas.includes(coleccion)) {
        return res.status(400).json({ msg: `las colecciones permitidas son las siguentes: ${coleccionesPermitidas}` });
    }
    ...
    switch (coleccion) {
        ...
        default:
            res.status(500).json({ msg: 'Se le olvido hecer busqueda' });
            break
    }
}
